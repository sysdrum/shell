...
var AutoTiler = class AutoTiler extends World {
    ...
    detach(ext, fork_entity, window, name) {
        let detach_entity = null;
        let detached = null;
        let retach = null;
        ...
        this.forks.with(fork_entity, (fork) => {
            if (fork.left.is_window(window)) {
                if (fork.right) {
                    log(`detaching ${name} (${window}) from ${fork_entity}`);
                    retach = fork.right;
                }

                log(`detaching ${name} (${window}) from ${fork_entity}`);
                detach_entity = fork_entity;
            } else if (fork.right && fork.right.is_window(window)) {
                log(`detaching ${name} (${window}) from ${fork_entity}`);
                fork.right = null;
                fork.tile(this, ext, fork.area);
            }
        });

        while (detach_entity) {
            detached = detach_entity.slice();
            detach_entity = null;

            for (const [entity, fork] of this.forks.iter()) {
                if (fork.left.is_fork(detached)) {
                    if (!retach && fork.right) {
                        log(`reassigning ${fork.right.entity}`);
                        retach = fork.right;
                    }

                    log(`detaching fork ${detached} from ${entity}`);
                    detach_entity = entity;

                    break
                } else if (fork.right && fork.right.is_fork(detached)) {
                    log(`detaching fork ${detached} from ${entity}`);
                    fork.right = retach;
                    retach = null;
                    fork.tile(this, ext, fork.area);

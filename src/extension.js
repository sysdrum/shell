...
var Ext = class Ext extends World {
    ...
    detach_window(win, name) {
        this.attached.take_with(win, (prev_fork) => {
            const reflow_fork = this.auto_tiler.detach(prev_fork, win, (fork, retached) => {
                log(`attaching window (${retached}) to (${fork})`);
                this.attached.insert(retached, fork);
            });

            if (reflow_fork) {
                reflow_fork.tile(this.auto_tiler, this, reflow_fork.area);
            }
    ...
    on_grab_end(meta, op) {
        ...
        if (win && this.grab_op && entity_eq(this.grab_op.entity, win.entity)) {
            ...
            if (movement[0] != 0 || movement[1] != 0) {
                log(`win: ${win.name()}; op: ${op}; from (${this.grab_op.rect.x},${this.grab_op.rect.y}) to (${crect.x},${crect.y})`);
